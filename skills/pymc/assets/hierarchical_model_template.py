...
def run_hierarchical_analysis() -> None:
    """
    Execute the full hierarchical modeling workflow.

    Includes data preparation, model building, prior/posterior predictive checks,
    diagnostic evaluation, and prediction for new data.
    """
    # =============================================================================
    # 1. DATA PREPARATION
    # =============================================================================
    ...
    with pm.Model(coords=coords) as hierarchical_model:
        ...
        beta = pm.Deterministic(
            "beta", mu_beta + sigma_beta * beta_offset, dims="groups"
        )
        ...
        # Likelihood
        pm.Normal("y_obs", mu=mu, sigma=sigma, observed=y_vals, dims="obs")
    ...
    with hierarchical_model:
        pm.set_data(
            {
                "X_data": new_x,
                "groups_data": new_groups,
                "obs": np.arange(len(new_x)),
            }
        )
    ...
    for i, g in enumerate(new_groups):
        print(
            f"{group_names[g]:<10} {new_x[i]:<10.2f} {y_pred_mean[i]:<15.3f} "
            f"{y_pred_hdi[i, 0]:<15.3f} {y_pred_hdi[i, 1]:<15.3f}"
        )
