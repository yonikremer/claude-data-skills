...
Skills are modular, self-contained packages that transform Gemini CLI from a general-purpose agent into a specialized agent equipped with procedural knowledge.
...
- **Description**: Describes ONLY when to use (triggering conditions), NOT what it does. **NEVER summarize the workflow in the description.**
