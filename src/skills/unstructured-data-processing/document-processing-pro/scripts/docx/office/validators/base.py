...
class BaseSchemaValidator:
    """
    Base class for document schema validation.

    Provides common logic for XML validation, namespace checking, ID uniqueness,
    ...
    def __init__(
        ...
        Args:
            ...
            original_file: Optional path to the original .docx/.pptx/.xlsx file for comparison.
    ...
    def validate(self) -> bool:
        """
        Perform all validation checks. Must be implemented by subclasses.
    ...
    def validate_xml(self) -> bool:
        ...
        Returns:
            True if all files are well-formed, False otherwise.
        """
        errors = []
    ...
    def validate_namespaces(self) -> bool:
        """
        Validate that all namespace prefixes used in Ignorable are properly declared.

        Returns:
            True if namespaces are valid, False otherwise.
        """
        errors = []
    ...
    def validate_unique_ids(self) -> bool:
        """
        Validate that all required IDs are unique within their defined scope.
    ...
    def validate_file_references(self) -> bool:
        """
        Validate all file references in .rels files and check for unreferenced files.

        Returns:
            True if all references are valid and all files are referenced, False otherwise.
        """
        errors = []
    ...
    def validate_all_relationship_ids(self) -> bool:
        """
        Validate all relationship IDs (r:id) in XML files.
    ...
    def _get_expected_relationship_type(self, element_name: str) -> Optional[str]:
        """
        Get the expected relationship type for an element name.

        Args:
            element_name: The name of the XML element.

        Returns:
            The expected relationship type or None.
        """
    ...
    def validate_content_types(self) -> bool:
        """
        Validate [Content_Types].xml declarations.
    ...
    def validate_file_against_xsd(
        ...
        Returns:
            A tuple of (is_valid, new_errors). is_valid is None if no schema exists.
        """
    ...
    def validate_against_xsd(self) -> bool:
        ...
        Returns:
            True if no new errors were found, False otherwise.
        """
        new_errors = []
    ...
    def _get_schema_path(self, xml_file: Path) -> Optional[Path]:
        """
        Get the path to the XSD schema for a given XML file.

        Args:
            xml_file: Path to the XML file.

        Returns:
            Path to the XSD schema, or None if no mapping exists.
        """
    ...
    def _remove_ignorable_elements(self, root: lxml.etree._Element) -> None:
        """
        Recursively remove ignorable elements from an XML element.
    ...
    def _validate_single_file_xsd(
        ...
        Returns:
            A tuple of (is_valid, errors). is_valid is None if no schema exists.
        """
    ...
    def _get_original_file_errors(self, xml_file: Path) -> Set[str]:
        """
        Get XSD validation errors from the original document for comparison.

        Args:
            xml_file: Path to the modified XML file.

        Returns:
            A set of error messages from the original file.
        """
    ...
    def _remove_template_tags_from_text_nodes(
        ...
        Returns:
            A tuple of (modified_xml_doc, warnings).
        """
        warnings = []
