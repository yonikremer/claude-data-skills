...
class BaseSchemaValidator:
    ...
    SCHEMA_MAPPINGS = {
        ...
        "commentsExtensible.xml": "microsoft/wml-cid-2016.xsd",
    ...
    def __init__(
        ...
        Args:
            ...
            original_file: Optional path to the original file for comparison.
    ...
    def repair(self) -> int:
        """Performs automatic repairs on the document.
    ...
    def repair_whitespace_preservation(self) -> int:
        """Repairs missing xml:space='preserve' attributes in w:t elements.
    ...
    def validate_xml(self) -> bool:
        ...
        Returns:
            True if all XML files are well-formed, False otherwise.
        """
        errors = []
    ...
    def validate_namespaces(self) -> bool:
        """Validates that all namespaces used in Ignorable are declared.

        Returns:
            True if all namespaces are valid, False otherwise.
        """
        errors = []
    ...
    def validate_unique_ids(self) -> bool:
        """Validates that all IDs required to be unique are unique.

        Returns:
            True if all IDs are unique, False otherwise.
        """
        errors = []
        global_ids: dict[str, tuple[Path, int, str]] = {}

        for xml_file in self.xml_files:
            try:
                root = lxml.etree.parse(str(xml_file)).getroot()
                file_ids: dict[tuple[str, str], dict[str, int]] = {}
    ...
    def validate_file_references(self) -> bool:
        """Validates that all file references in .rels files are valid and all files are referenced.
    ...
    def validate_all_relationship_ids(self) -> bool:
        """Validates that all relationship IDs in XML files exist in the corresponding .rels file.

        Returns:
            True if all relationship IDs are valid, False otherwise.
        """
        import lxml.etree
    ...
    def _get_expected_relationship_type(self, element_name: str) -> str | None:
        """Gets the expected relationship type for an element.

        Args:
            element_name: The name of the element.

        Returns:
            The expected relationship type, or None if unknown.
        """
    ...
    def validate_content_types(self) -> bool:
        """Validates that all content files are declared in [Content_Types].xml.

        Returns:
            True if all files are declared, False otherwise.
        """
        errors = []
    ...
    def validate_file_against_xsd(
        ...
        Returns:
            A tuple of (is_valid, errors_set). is_valid is None if no schema found.
        """
    ...
    def validate_against_xsd(self) -> bool:
        """Validates all XML files against their XSD schemas.

        Returns:
            True if no new XSD validation errors are found, False otherwise.
        """
        new_errors = []
    ...
    def _get_schema_path(self, xml_file: Path) -> Path | None:
        """Gets the path to the XSD schema for a given XML file.

        Args:
            xml_file: Path to the XML file.

        Returns:
            Path to the schema, or None if no schema found.
        """
    ...
    def _clean_ignorable_namespaces(
        ...
        Args:
            xml_doc: The XML document to clean.

        Returns:
            The cleaned XML document.
        """
    ...
    def _remove_ignorable_elements(self, root: lxml.etree._Element) -> None:
        """Recursively removes ignorable elements from an XML tree.
    ...
    def _preprocess_for_mc_ignorable(
        ...
        Args:
            xml_doc: The XML document to preprocess.

        Returns:
            The preprocessed XML document.
        """
    ...
    def _validate_single_file_xsd(
        ...
        Args:
            xml_file: Path to the XML file.
            base_path: Base path of the unpacked document.
    ...
    def _get_original_file_errors(self, xml_file: Path) -> set[str]:
        """Gets XSD validation errors from the corresponding file in the original document.

        Args:
            xml_file: Path to the modified XML file.
    ...
    def _remove_template_tags_from_text_nodes(
        ...
        Args:
            xml_doc: The XML document to process.

        Returns:
            A tuple of (processed_xml_doc, list_of_warnings).
        """
        warnings = []
