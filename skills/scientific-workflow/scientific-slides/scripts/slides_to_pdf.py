...
def get_image_files(paths: List[str]) -> List[Path]:
    ...
    Args:
        paths (List[str]): List of file paths or directory paths.

    Returns:
        List[Path]: Sorted list of image file paths.
    """
    image_extensions: set = {".png", ".jpg", ".jpeg", ".gif", ".webp", ".bmp"}
    ...
    for path_str in paths:
        ...
        if path.is_file():
            ...
            if parent.exists():
                matches: List[Path] = list(parent.glob(pattern))
...
def combine_images_to_pdf(
    ...
    Args:
        ...
        dpi (int): Resolution for the PDF (default: 150).
    ...
    for i, img_path in enumerate(image_paths):
        try:
            ...
            if verbose:
                print(
                    f"  [{i + 1}/{len(image_paths)}] Loaded: {img_path.name} "
    ...
    try:
        ...
        remaining_images: List[Image.Image] = images[1:] if len(images) > 1 else []
...
Tips:
  ...
  - Standard presentation aspect ratio is 16:9 (1920x1080 or 1280x720)
    ...
    # Get image files
    image_files: List[Path] = get_image_files(args.images)
    ...
    success: bool = combine_images_to_pdf(
