...
def get_cpu_info() -> Dict[str, Any]:
    ...
    Returns:
        Dict[str, Any]: A dictionary containing CPU details such as physical cores,
            logical cores, max frequency, architecture, and processor name.
    """
...
def get_memory_info() -> Dict[str, Any]:
    ...
    Returns:
        Dict[str, Any]: A dictionary containing memory details such as total GB,
            available GB, used GB, percent used, and swap memory information.
    """
...
def get_disk_info(path: Optional[str] = None) -> Dict[str, Any]:
    ...
    Args:
        path (Optional[str]): The path to check disk usage for. Defaults to the
            current working directory if None.

    Returns:
        Dict[str, Any]: A dictionary containing disk usage details such as path,
            total GB, available GB, used GB, and percent used. If an error occurs,
            the dictionary will contain the path and an error message.
    """
    if path is None:
        path = os.getcwd()
...
def detect_nvidia_gpus() -> List[Dict[str, Any]]:
    ...
    Returns:
        List[Dict[str, Any]]: A list of dictionaries, each containing details for
            an identified NVIDIA GPU, such as index, name, memory, driver version,
...
def detect_apple_silicon_gpu() -> Optional[Dict[str, Any]]:
    ...
    Returns:
        Optional[Dict[str, Any]]: A dictionary containing Apple Silicon GPU details
...
def get_gpu_info() -> Dict[str, Any]:
    ...
    Returns:
        Dict[str, Any]: A dictionary summarizing detected GPUs across different
            platforms (NVIDIA, AMD, Apple Silicon).
    """
    gpu_info: Dict[str, Any] = {
...
def get_os_info() -> Dict[str, Any]:
    ...
    Returns:
        Dict[str, Any]: A dictionary containing OS details such as system, release,
            version, machine type, and Python version.
    """
    return {
...
def detect_all_resources(output_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    Args:
        output_path (Optional[str]): Optional path to save JSON. Defaults to
            .claude_resources.json in the current working directory.
...
def generate_recommendations(resources: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Args:
        resources (Dict[str, Any]): A dictionary containing detected resource info.

    Returns:
        Dict[str, Any]: A dictionary containing recommendations for parallel
            processing, memory strategy, GPU acceleration, and large data handling.
    """
    ...
    # CPU recommendations
    cpu_cores: int = resources["cpu"]["logical_cores"]
    if cpu_cores >= 8:
    ...
    available_memory_gb: float = resources["memory"]["available_gb"]
    ...
    # GPU recommendations
    gpu_info: Dict[str, Any] = resources["gpu"]
    ...
    disk_available_gb: float = resources["disk"]["available_gb"]
...
def main() -> None:
    ...
    print(
        f"  CPU: {resources['cpu']['logical_cores']} cores "
        f"({resources['cpu']['physical_cores']} physical)"
    )
    print(
    ...
    print(
        f"  Disk: {resources['disk']['total_gb']} GB total, "
        f"{resources['disk']['available_gb']} GB available"
    )

    if resources["gpu"]["total_gpus"] > 0:
        print(
            f"  GPU: {resources['gpu']['total_gpus']} detected "
            f"({', '.join(resources['gpu']['available_backends'])})"
        )
    else:
    ...
    print(
        f"  GPU Acceleration: "
        f"{'Available' if recs['gpu_acceleration'].get('available') else 'Not Available'}"
    )
