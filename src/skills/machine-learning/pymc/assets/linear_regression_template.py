...
def run_linear_regression() -> None:
    """
    Execute the full linear regression workflow.

    Includes data preparation (with standardization), model building,
    prior/posterior predictive checks, diagnostic evaluation,
    coefficient analysis, and prediction for new data.
    """
    # =============================================================================
    # 1. DATA PREPARATION
    # =============================================================================
    ...
    x_vals = np.random.randn(n_samples, n_predictors)
    true_beta = np.array([1.5, -0.8, 2.1])
    true_alpha = 0.5
    y_vals = true_alpha + x_vals @ true_beta + np.random.randn(n_samples) * 0.5
    ...
    coords = {"predictors": predictor_names, "obs_id": np.arange(len(y_vals))}
    ...
    with pm.Model(coords=coords) as linear_model:
        ...
        # Likelihood
        pm.Normal("y_obs", mu=mu, sigma=sigma, observed=y_vals, dims="obs_id")
