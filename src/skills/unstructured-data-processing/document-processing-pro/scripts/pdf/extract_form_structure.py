...
def extract_form_structure(pdf_path: str) -> Dict[str, Any]:
    """Extracts structural elements (labels, lines, checkboxes) from a PDF.

    Analyzes a non-fillable PDF to identify text labels, horizontal lines that
    define row boundaries, and small rectangles that likely represent checkboxes.

    Args:
        pdf_path: Path to the input PDF file.

    Returns:
        A dictionary containing extracted pages, labels, lines, checkboxes,
        and row boundaries.
    """
    ...
    for line in structure["lines"]:
        page_idx = line["page"]
        if page_idx not in lines_by_page:
            lines_by_page[page_idx] = []
        lines_by_page[page_idx].append(line["y"])

    for page_idx, y_coords in lines_by_page.items():
        sorted_coords = sorted(set(y_coords))
        for i in range(len(sorted_coords) - 1):
            structure["row_boundaries"].append(
                {
                    "page": page_idx,
                    "row_top": sorted_coords[i],
                    "row_bottom": sorted_coords[i + 1],
                    "row_height": round(sorted_coords[i + 1] - sorted_coords[i], 1),
                }
            )
...
def main() -> None:
    """Main entry point for extracting PDF form structure."""
    if len(sys.argv) != 3:
