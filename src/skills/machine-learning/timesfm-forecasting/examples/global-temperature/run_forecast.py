...
def main() -> None:
    """Run the TimesFM forecast on global temperature anomaly data.

    Loads the temperature anomaly data, initializes the TimesFM model,
    generates a 12-month forecast, and saves the results in CSV and JSON formats.
    """
    # Preflight check
    print("=" * 60)
    ...
    print(
        f"   vs 2024 mean:   {point.mean() - df['anomaly_c'].iloc[-12:].mean():+.3f}°C"
    )
