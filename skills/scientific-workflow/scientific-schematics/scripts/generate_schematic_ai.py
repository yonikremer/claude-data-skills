...
class ScientificSchematicGenerator:
    ...
    Attributes:
        QUALITY_THRESHOLDS (Dict[str, float]): Quality thresholds by document type.
        SCIENTIFIC_DIAGRAM_GUIDELINES (str): Guidelines for scientific diagrams.
    """
    ...
    SCIENTIFIC_DIAGRAM_GUIDELINES: str = """
...
"""
    ...
    def __init__(self, api_key: Optional[str] = None, verbose: bool = False) -> None:
        ...
        self.image_model: str = "google/gemini-3.1-flash-image-preview"
        # Gemini 3.1 Pro Preview for quality review - excellent vision and reasoning
        self.review_model: str = "google/gemini-3.1-pro-preview"
        ...
        try:
            ...
            if self.verbose:
                ...
                if "choices" in response and response["choices"]:
                    msg: Dict[str, Any] = response["choices"][0].get("message", {})
    ...
    def review_image(
        ...
        Args:
            ...
            doc_type (str): Document type (journal, poster, presentation, etc.).
            max_iterations (int): Maximum iterations allowed.

        Returns:
            Tuple[str, float, bool]: Tuple of (critique text, quality score 0-10,
        ...
        threshold: float = self.QUALITY_THRESHOLDS.get(
            doc_type.lower(), self.QUALITY_THRESHOLDS["default"]
        )

        review_prompt: str = f"""You are an expert reviewer evaluating a scientific diagram for publication quality.
...
If score < {threshold}, mark as NEEDS_IMPROVEMENT with specific suggestions."""
        ...
        try:
            ...
            score: float = 7.5  # Default score if extraction fails
            import re
    ...
    def improve_prompt(
        ...
        Args:
            original_prompt (str): Original user prompt.
            critique (str): Review critique from previous iteration.
            iteration (int): Current iteration number.

        Returns:
            str: Improved prompt for next generation.
        """
        improved_prompt: str = f"""{self.SCIENTIFIC_DIAGRAM_GUIDELINES}
...
Generate an improved version that addresses all the critique points while maintaining scientific accuracy and professional quality."""
    ...
    def generate_iterative(
        ...
        Args:
            user_prompt (str): User's description of desired diagram.
            output_path (str): Path to save final image.
            iterations (int): Maximum refinement iterations (default: 2, max: 2).
            doc_type (str): Document type for quality threshold (journal, poster, etc.).
        ...
        current_prompt: str = f"""{self.SCIENTIFIC_DIAGRAM_GUIDELINES}
...
Generate a publication-quality scientific diagram that meets all the guidelines above."""
        ...
        for i in range(1, iterations + 1):
            ...
            image_data: Optional[bytes] = self.generate_image(current_prompt)

            if not image_data:
                error_msg: str = getattr(
                    self,
                    "_last_error",
            ...
            iter_path: Path = output_dir / f"{base_name}_v{i}{extension}"
        ...
        if results["success"] and results["final_image"]:
            final_iter_path: Path = Path(results["final_image"])
            if str(final_iter_path) != output_path:
                import shutil
        ...
        # Save review log
        log_path: Path = output_dir / f"{base_name}_review_log.json"
...
Environment:
  OPENROUTER_API_KEY    OpenRouter API key (required)
    ...
    try:
        ...
        results: Dict[str, Any] = generator.generate_iterative(
